...
for n in range(1, N+1):
    ...
    run_one_image(img, model_mae, params, key=rng, epochs=num_epochs, dataset_name=dataset_name.upper(), model_arch=model_arch, suffix=f"train{n}")
...
for n in range(1, N+1):
    ...
    img = next(test_it)[0]
    # run the MAE model on the selected image and save the results to a .png file
    run_one_image(img, model_mae, params, key=rng, epochs=num_epochs, dataset_name=dataset_name.upper(), model_arch=model_arch, suffix=f"test{n}")
