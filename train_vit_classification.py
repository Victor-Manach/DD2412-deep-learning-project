...
class TrainModule:
    def __init__(self, model, exmp_imgs, dataset_name, model_arch, num_epochs, num_steps_per_epoch, seed=42):
        super().__init__()
        self.seed = seed
        ...
        self.log_dir = os.path.join(CHECKPOINT_PATH, dataset_name, model_arch)
        self.dataset_name = dataset_name
        # Create jitted training and eval functions
        self.create_functions()
        # Initialize model
        self.init_model(exmp_imgs, num_epochs, num_steps_per_epoch)
    ...
    def create_functions(self):
        ...
        def compute_loss(params, key, batch, train):
            ...
            key, dropout_apply_rng, drop_path_apply_rng = jax.random.split(key, 3)
            logits = self.model.apply({"params": params}, x=imgs, train=train, rngs={"dropout": dropout_apply_rng, "drop_path": drop_path_apply_rng})
    ...
    def init_model(self, exmp_imgs, num_epochs, num_steps_per_epoch):
        """ Initialize the MAE model with the proper random keys, the learning rate and the optimizer.
        """
        # Initialize model
        self.rng = jax.random.PRNGKey(self.seed)
        self.rng, init_rng, dropout_init_rng, drop_path_init_rng = jax.random.split(self.rng, 4)
        params = self.model.init({"params": init_rng, "dropout": dropout_init_rng, "drop_path": drop_path_init_rng}, x=exmp_imgs, train=True)["params"]
    ...
    def train_model(self, train_data, val_data, num_epochs):
        ...
        for epoch_idx in range(1, num_epochs+1):
            ...
            pbar.update(1)
            if epoch_idx % 50 == 0:
                self.save_model(step=epoch_idx) # save the model every 50 epochs
